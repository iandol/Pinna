...
sM.movieSettings.size = [800 800];
sM.movieSettings.fps = screenVals.fps;
sM.movieSettings.codec = 'theoraenc';
sM.movieSettings.quality = 1; %quality vs. size max==1
sM.movieSettings.keyframe = 2;
sM.movieSettings.nFrames = screenVals.fps * 4; % 3 second recording
sM.prepareMovie();
...
if ~exist('radial_speed','var'); radial_speed = 8;end
...
radius_modifier = 0.5; % used to calculate the radius position
size_modifier = 1; %used to modify the size increase
...
contrast = 100.0;
aspectratio = 1.0;
Gabor_parameters = repmat([tilt, freq, sc, contrast, aspectratio, 0, 0, 0]',1,patchNum_perRing*ring_num);
...
try
	...
	while ii < sti_duration * screenVals.fps
		...
		for i = 1:ring_num
			...
			R = min_radius_pixel + radius_modifier*diff_r*n_radial(i)^2;
			S = min_patch_size + size_modifier*diff_size*n_radial(i)^2;
